...
export default async function RequestDetailPage({ params }) {
  // 1. Unwrap the asynchronous params object
  ...
  // 2. Use the extracted requestId variable for database lookups
      ...
      <div className="glass-card rounded-xl p-5 mb-4">
        ...
        {!isTerminal && (
          <div className="border-t pt-4" style={{borderColor:'rgba(245,166,35,0.1)'}}>
            ...
            <ApprovalActions requestId={req.id} currentStatus={req.status} userRole={profile?.role} />
          </div>
        )}
        {isTerminal && (
          <div className="border-t pt-4" style={{borderColor:'rgba(245,166,35,0.1)'}}>
            <div className={`rounded-lg p-3 text-sm ${req.status === 'approved' ? '' : ''}`}
              style={{
